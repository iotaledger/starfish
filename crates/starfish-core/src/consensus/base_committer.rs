...
/// The [`BaseCommitter`] contains the bare bone commit logic.
/// Once instantiated, the method `try_direct_decide` and
/// `try_indirect_decide` can be called at any time and any
/// number of times (it is idempotent) to determine whether a
/// leader can be committed or skipped.
#[derive(Clone)]
...
impl BaseCommitter {
    ...
    /// Return the leader round of the specified wave number. The leader round
    /// is always the first round of the wave.
    ...
    /// The leader-elect protocol is offset by `leader_offset`
    /// to ensure that different committers with different leader
    /// offsets elect different leaders for the same round number.
    /// This function returns `None` if there are no leaders for
    /// the specified round.
    ...
    /// to see whether we can indirect-commit or indirect-skip it.
    ...
    /// Apply the direct decision rule to the specified leader to
    /// see whether we can direct-commit or direct-skip it.
    #[tracing::instrument(skip_all, fields(leader = %format_authority_round(leader, leader_round)))]
