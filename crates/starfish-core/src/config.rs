...
pub struct NodeParameters {
    ...
    /// Fresh, nonzero execution identifier shared by every validator in one
    /// Starfish-RBC run. It is generated by benchmark setup and ignored by all
    /// other protocols.
    #[serde(default)]
...
impl NodeParameters {
    ...
    /// Replace the Starfish-RBC execution identifier with fresh OS randomness.
    ///
    /// The orchestrator calls this once before serializing the shared node
    /// parameters, so independently generated benchmark files on remote hosts
    /// still contain the same identifier.
    pub fn refresh_starfish_rbc_protocol_instance(&mut self) -> [u8; 32] {
        let mut rng = OsRng;
        let protocol_instance = loop {
            let mut bytes = [0; 32];
        ...
        self.starfish_rbc_protocol_instance = Some(protocol_instance);
        protocol_instance
    }
}
